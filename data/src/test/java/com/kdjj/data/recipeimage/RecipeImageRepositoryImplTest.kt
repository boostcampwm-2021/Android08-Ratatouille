...
class RecipeImageRepositoryImplTest {
    ...
    lateinit var mockRecipeImageLocalDataSource: RecipeImageLocalDataSource
    lateinit var mockRecipeImageRemoteDataSource: RecipeImageRemoteDataSource
    lateinit var recipeImageRepositoryImpl: RecipeImageRepositoryImpl
    ...
    fun convertToByteArrayRemote_getRemoteImageByteArray_true(): Unit = runBlocking {
        //given
        `when`(mockRecipeImageRemoteDataSource.fetchRecipeImage(testUri)).thenReturn(Result.success(testByteArray))
        //when
    ...
    fun convertToByteArrayLocal_getLocalImageByteArray_true(): Unit = runBlocking {
        //given
        `when`(mockRecipeImageLocalDataSource.convertToByteArray(testUri)).thenReturn(Result.success(testByteArray))
        //when
    ...
    fun convertToRemoteStorageUri_getRemoteImageUri_true(): Unit = runBlocking {
        //given
        `when`(mockRecipeImageRemoteDataSource.uploadRecipeImage(testUri)).thenReturn(Result.success(testUri))
        //when
    ...
    fun convertToLocalStorageUri_getLocalImageUri_true(): Unit = runBlocking {
        //given
        `when`(mockRecipeImageLocalDataSource.convertToInternalStorageUri(testByteArray, "fileName")).thenReturn(Result.success(testUri))
        //when
        val newImagePathResult = recipeImageRepositoryImpl.convertToLocalStorageUri(testByteArray, "fileName")
        //then
        assertEquals(testUri, newImagePathResult.getOrNull())
    }
}